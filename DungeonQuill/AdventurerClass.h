...
public:
	BasicInfo _classInfo;		//职业信息
	BasicInfo subClassInfo;		//职业分支信息
	std::vector<BasicInfo> spellList;		//法术列表
	std::vector<BasicInfo> traitList;	//职业特性列表

	AdventurerClass(int tid, std::string tname, std::string tremarks, 
		int ttid, std::string ttname, std::string ttremarks, int tlevel,
		...
		_classInfo(tid,tname,tremarks),
		subClassInfo(ttid, ttname, ttremarks),
		level(tlevel), 
		specialAbilityName(tspecialAbilityName),
